...
```bash
# Load platform-specific epic sync rules if available
case "$PLATFORM_TYPE" in
  "yunxiao"|"alicloud")
    if [ -f ".claude/rules/platform-yunxiao-epic.md" ]; then
      echo "Loading Yunxiao platform epic sync rules..."
      ...
      echo "Warning: Yunxiao platform selected but epic rules not found. Falling back to GitHub."
      PLATFORM_TYPE="github"
    fi
    ;;
  "github"|*)
    echo "Using GitHub epic sync logic (default)"
    ;;
esac
```
...
if [ "$PLATFORM_TYPE" = "yunxiao" ] || [ "$PLATFORM_TYPE" = "alicloud" ]; then
  ...
  echo "Initializing Yunxiao epic sync for project: $PLATFORM_PROJECT_ID"

  # TODO: Load Yunxiao-specific epic creation logic from rule files
  # For now, show that routing works and fall back to GitHub
  echo "⚠️ Yunxiao epic sync not yet implemented. Falling back to GitHub for now."
  PLATFORM_TYPE="github"
fi
```
